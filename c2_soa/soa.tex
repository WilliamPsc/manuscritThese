...
In the state of the art, multiple solutions exist to protect a system against FIA such as Error Correcting Code (ECC), spatial or hardware redundancy, temporal redundancy, and obfuscation.

Spatial redundancy~\cite{BCNTW-06-procieee, PTAJC-22-appSci} countermeasure consists of duplicating the protected circuit or part of it to compare the result obtained after computation to check if there is a difference. Figure~\ref{fig:spatial_redundancy} represents the spatial redundancy. An input is sent to two modules (i.e. computation blocks) and the output results will be compared, to check if an error occured.
...
Temporal redundancy~\cite{BCNTW-06-procieee, PTAJC-22-appSci}
...
Simple parity is a class of countermeasure that only compute the parity (odd or even) of the protected target (i.e. register). This method can only detect an error without the possibility of correcting it. This method will be developed in Chapter~\ref{chapter:countermeasures}.
