...
We present the most popular fault models, which have been used extensively in the literature.

Multiples studies~\cite{BH-22-access,GMA-22-electronics,KSV-13-tvlsi,ZAV-06-jarab,YSW-18-hss} present a small overview on different fault model for Fault Injection Attacks. Different possibilities exist depending on the equipment and the effect targeted. Otto~\cite{O-05-phd} presented a deep study and definitions of fault models.
...
Hardware countermeasures consist of adding hardware mechanisms to the system architecture, which makes them more effective. Adding a countermeasure introduces a loss of performance into the target system. Its implementation usually involves increasing the size of the hardware's area, reducing the maximum frequency, or increasing the power consumption. Once the implementation is done, an evaluation of the protection is usually done to compare it and give some indications in terms of area, performance, or efficiency.
In the state of the art, multiple solutions exist to protect a system against FIA such as Error Correcting Code (ECC), spatial redundancy, temporal redundancy, or obfuscation.

Spatial redundancy

Manssour et al.~\cite{NLGT-22-ddecs}

Simple parity is a class of countermeasure that only add one bit to the protected target (i.e. register).

Error Correction Code (ECC) (hamming code, secded, Reed-Salomon, LDPC, BCH)
